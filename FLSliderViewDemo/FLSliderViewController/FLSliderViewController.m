...
#define kFLDeviceWidth [UIScreen mainScreen].bounds.size.width
#define kFLDeviceHeight [UIScreen mainScreen].bounds.size.height
...
static CGFloat const rightShowWidth = 240.f;//右边显示的宽度
static CGFloat const rightScale = 1.f;//缩放比例
static CGFloat const rightDragbleWidth = 80.f;
static CGFloat const rightMinDragLength = 100.f;

typedef NS_ENUM(NSUInteger, FLDragDirection){
    FLDragDirectionNone = 0,
    FLDragDirectionLeft, // 左侧界面相关  show the leftView or hide
    FLDragDirectionRight,
};

@interface FLSliderViewController () <UIGestureRecognizerDelegate>
{
    ...
    FLDragDirection _dragDirection;
...
@implementation FLSliderViewController
...
- (BOOL)gestureRecognizerShouldBegin:(UIGestureRecognizer *)gestureRecognizer
{
    ...
    // 判断点击拖动手势是否在允许拖动范围内
    if ([gestureRecognizer locationInView:_mainContainerView].x < leftDragbleWidth || [gestureRecognizer locationInView:_mainContainerView].x > kFLDeviceWidth-rightDragbleWidth) {
        return YES;
    }
    return NO;
}
    ...
    switch (gesture.state) {
        case UIGestureRecognizerStateBegan:
        {
            ...
            if (!_isLeftShow&&!_isRightShow) {
                if (point.x<leftDragbleWidth||point.x>kFLDeviceWidth-rightDragbleWidth) {
                    _canDrag = YES;
                }else{
                    _canDrag = NO;
                }
        ...
        case UIGestureRecognizerStateChanged:
        {
            ...
            if (!_isLeftShow&&!_isRightShow) {
                if (_dragDirection == FLDragDirectionNone) {
                    if (move_length>0) {
                        _dragDirection = FLDragDirectionLeft;
                        _leftContainerView.hidden = NO;
                        _rightContainerView.hidden = YES;
                    }else{
                        _dragDirection = FLDragDirectionRight;
                        _leftContainerView.hidden = YES;
                        _rightContainerView.hidden = NO;
                    }
                }
                ...
                switch (_dragDirection) {
                    case FLDragDirectionLeft:
                    {
                        if (!_canShowLeft) {
                            break;
                        }
                        
                        if (_startDragPoint.x>kFLDeviceWidth-rightDragbleWidth||main_x+move_length<0) {
                            // 防止拖拉左侧区域可以拖出右侧界面
                            break;
                        }
        ...
        case UIGestureRecognizerStateEnded:
        {
            ...
            _dragDirection = FLDragDirectionNone;
...
- (NSTimeInterval)calculateAnimationDurationIsShow:(BOOL)isShow
{
    ...
    if (main_x>0) {
        // left
        CGFloat left_scale = _leftContainerView.frame.size.width/kFLDeviceWidth;
        if (isShow) {
        ...
        // right
        CGFloat right_scale = _rightContainerView.frame.size.width/kFLDeviceWidth;
        if (isShow) {
